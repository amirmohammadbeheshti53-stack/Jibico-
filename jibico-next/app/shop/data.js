...
.cats{display:grid;grid-template-columns:repeat(4,1fr);gap:18px;margin-bottom:60px}
.cat{background:var(--navy2);border:1px solid var(--line);border-radius:20px;padding:30px 20px;text-align:center;transition:.3s;text-decoration:none;color:inherit;display:block;position:relative;overflow:hidden}
.cat::before{content:'';position:absolute;top:0;right:0;width:100%;height:4px;background:var(--grad-gold);transform:scaleX(0);transition:.4s;transform-origin:right}
.cat:hover::before{transform:scaleX(1)}
.cat:hover{border-color:var(--gold);transform:translateY(-8px)}
.cat .c-ico{font-size:44px;margin-bottom:12px}
.cat h3{font-size:16px;margin-bottom:6px}
.cat p{color:var(--muted);font-size:12px}
.cat .c-go{display:inline-block;margin-top:12px;color:var(--gold);font-size:13px;font-weight:800}
...
.mini-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-bottom:40px}
...
.mini-card .m-price{font-size:19px;font-weight:900;color:var(--gold);margin:14px 0}
...
.all-link{text-align:center;margin-bottom:60px}
.all-link a{color:var(--gold);font-weight:800;font-size:15px;text-decoration:none;border-bottom:2px solid var(--gold)}
