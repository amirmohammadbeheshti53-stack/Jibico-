...
.prod{background:var(--navy2);border:1px solid var(--line);border-radius:24px;padding:40px;margin-bottom:35px;position:relative;overflow:hidden}
.prod::before{content:'';position:absolute;top:0;right:0;width:100%;height:4px;background:var(--grad-gold)}
.prod-head{display:flex;gap:20px;align-items:center;flex-wrap:wrap;margin-bottom:20px}
.prod-head .p-ico{width:80px;height:80px;background:var(--gold-soft);border:1px solid rgba(242,182,50,.4);border-radius:20px;display:flex;align-items:center;justify-content:center;font-size:40px;flex-shrink:0}
.prod-head h2{font-size:clamp(20px,3vw,26px);font-weight:900}
.prod-head .sub{color:var(--gold);font-size:14px;font-weight:700}
.prod-head .price-tag{margin-right:auto;background:var(--navy);border:1px dashed var(--gold);border-radius:14px;padding:10px 22px;text-align:center}
.price-tag b{display:block;color:var(--gold);font-size:20px;font-weight:900}
.price-tag span{font-size:11px;color:var(--muted)}
.prod .desc{color:#cbd5e1;font-size:15px;margin-bottom:24px}
.prod-cols{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-bottom:26px}
.pcol{background:var(--navy);border:1px solid var(--line);border-radius:16px;padding:20px}
.pcol h4{font-size:14px;color:var(--gold);margin-bottom:12px}
.pcol ul{list-style:none}
.pcol li{font-size:13px;color:#cbd5e1;padding:6px 0;border-bottom:1px dashed var(--line)}
.pcol li:last-child{border:none}
.pcol.no li{color:var(--muted)}
.prod-actions{display:flex;gap:12px;flex-wrap:wrap;align-items:center}
.ord-btn{display:inline-block;background:var(--grad-gold);color:var(--navy);padding:14px 34px;border-radius:12px;font-weight:800;font-size:15px;text-decoration:none;transition:.3s}
.ord-btn:hover{transform:translateY(-4px);box-shadow:0 15px 35px rgba(242,182,50,.3)}
.note-mini{color:var(--muted);font-size:12px}
.bundle{background:var(--grad-gold);border-radius:24px;padding:40px;color:var(--navy);text-align:center;margin:50px 0;position:relative;overflow:hidden}
.bundle .b-badge{position:absolute;top:18px;left:18px;background:var(--navy);color:var(--gold);border-radius:50px;padding:6px 18px;font-size:12px;font-weight:800}
.bundle h2{font-size:clamp(22px,3.5vw,30px);font-weight:900;margin-bottom:10px}
.bundle p{font-size:15px;margin-bottom:20px;max-width:640px;margin-left:auto;margin-right:auto}
.bundle .b-price{font-size:30px;font-weight:900;margin-bottom:6px}
.bundle .b-price s{font-size:16px;opacity:.6;margin-left:10px}
.bundle .b-save{display:inline-block;background:rgba(10,26,47,.15);border-radius:50px;padding:6px 20px;font-size:13px;font-weight:800;margin-bottom:22px}
.bundle .ord-btn{background:var(--navy);color:var(--gold)}
.mini-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-bottom:50px}
.mini-card{background:var(--navy2);border:1px solid var(--line);border-radius:20px;padding:28px;display:flex;flex-direction:column;transition:.3s;position:relative}
.mini-card:hover{border-color:var(--gold);transform:translateY(-8px)}
.mini-card .m-badge{position:absolute;top:16px;left:16px;background:var(--gold-soft);border:1px solid rgba(242,182,50,.4);color:var(--gold);border-radius:50px;padding:3px 14px;font-size:11px;font-weight:800}
.mini-card .m-ico{font-size:44px;margin-bottom:14px}
.mini-card h3{font-size:17px;margin-bottom:4px}
.mini-card .m-sub{color:var(--gold);font-size:12px;font-weight:700;margin-bottom:12px}
.mini-card p{color:var(--muted);font-size:13px;flex:1}
.mini-card ul{list-style:none;margin:14px 0}
.mini-card li{font-size:12px;color:#cbd5e1;padding:5px 0;border-bottom:1px dashed var(--line)}
.mini-card li:last-child{border:none}
.mini-card .m-price{font-size:19px;font-weight:900;color:var(--gold);margin-bottom:14px}
.mini-card .m-price small{font-size:11px;color:var(--muted);font-weight:400}
.mini-card .ord{display:block;background:var(--grad-gold);color:var(--navy);border-radius:12px;padding:11px;text-align:center;font-weight:800;font-size:14px;text-decoration:none;transition:.3s}
.mini-card .ord:hover{transform:translateY(-3px);box-shadow:0 10px 25px rgba(242,182,50,.3)}
.guarantee{background:var(--navy2);border:1px solid rgba(74,222,128,.3);border-radius:16px;padding:20px;text-align:center;color:#4ade80;font-weight:700;margin-bottom:60px}
...
@media (max-width:980px){.menu{display:none}.burger{display:flex}.topbar{display:flex}.search-console{display:none}.prod-cols,.mini-grid{grid-template-columns:1fr}.footer-top{grid-template-columns:1fr 1fr}.footer-bottom{justify-content:center;text-align:center}}
@media (max-width:640px){.announce{flex-wrap:nowrap;justify-content:flex-start;gap:8px;padding:9px 12px 9px 34px}.announce .an-text{flex:1;min-width:0;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.announce .an-text b{display:none}.an-btn{font-size:11px;padding:6px 14px}.announce .close-a{left:6px;width:22px;height:22px;font-size:14px}.announce-form input{width:100%}.footer-top{grid-template-columns:1fr}.prod{padding:25px}.prod-head .price-tag{margin-right:0;width:100%}.bundle{padding:28px 20px}.bundle .b-badge{position:static;display:inline-block;margin-bottom:14px}}
`;
export const html = `
...
<div class="wrap">
    ...
    <div class="bundle reveal">
        ...
        <a class="ord-btn" href="https://ble.ir/jibicoclub" target="_blank">سفارش پکیج از طریق بله</a>
    </div>
    ...
    <div class="prod reveal">
        ...
        <div class="prod-actions">
            <a class="ord-btn" href="https://ble.ir/jibicoclub" target="_blank">🛒 سفارش از طریق بله</a>
